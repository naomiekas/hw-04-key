...
**The majors with the lowest unemployment rates are Math & COmputer Science, Military Technologies, Botany, Soil Science, Ed Admin**
...
**The top three majors are Early Childhood Education, Communication Disorders, Medical Assisting**
...
**We often choose the median because it represents the most common and is less susceptible to outliers**
...
**I would choose 5000 because it is a bit smoother and allows you to better see the distribution. 1000 was a bit too jagged**
...
4.  Based on the shape of the histogram you created in the previous exercise, determine which of these summary statistics is useful for describing the distribution.
    ...
    **I would eliminate the max as it is clearly an outlier and there are little values at the min. The mean is likely being pulled because of that max value. The median is probably the best choice.**
...
**The highest typical median income is engineering**
...
**The least popular major is interdisciplinary**
...
```{r}
college_recent_grads%>%
  ggplot(aes(x = sharewomen, y = median, color = major_type)) + 
  geom_point()
```

**As the share of women increased the median income decreases**
